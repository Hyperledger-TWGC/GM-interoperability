...
func MarshalECSM2PrivateKey(key *PrivateKey,pwd []byte) ([]byte,error){
	priv:=tjfoc_gm_sm2.PrivateKey{}
	priv.X=key.X
	priv.Y=key.Y
	priv.D=key.D
	priv.Curve=key.Curve
	return tjfoc_gm_x509.MarshalSm2EcryptedPrivateKey(&priv,pwd)
}

func ParsePKISM2PrivateKey(der []byte) (*PrivateKey, error) {
	priv,err:=tjfoc_gm_x509.ParseSm2PrivateKey(der)
	if err != nil{
		return nil, err
	}
	key:=PrivateKey{}
	...
	key.Y=priv.Y
	key.D=priv.D
	key.Curve=priv.Curve
	return &key,err

}

func ParsePKCS8PrivateKey(der, pwd []byte) (*PrivateKey, error) {
	priv,err:=tjfoc_gm_x509.ParsePKCS8PrivateKey(der,pwd)
...
func ParsePKCS8EcryptedPrivateKey(der, pwd []byte) (*PrivateKey, error) {
	...
	priv,err:=tjfoc_gm_x509.ParsePKCS8EcryptedPrivateKey(der,pwd)
	if err != nil{
		return nil, err
	}
	key:=PrivateKey{}
...
func WritePublicKeytoMem(key *PublicKey,pwd []byte) ([]byte, error) {
	pub:=tjfoc_gm_sm2.PublicKey{}
	pub.X=key.X
	pub.Y=key.Y
	pub.Curve=key.Curve
	return writePublicKeytoMem(&pub,pwd)
...
func ReadPrivateKeyFromMem(data []byte, pwd []byte) (*PrivateKey, error) {
	priv,err:=readPrivateKeyFromMem(data,pwd)
	if err != nil{
		return nil, err
	}
	key:=PrivateKey{}
...
func ReadPrivateKeyFromPem(FileName string, pwd []byte) (*PrivateKey, error) {
	priv,err:=tjfoc_gm_x509.ReadPrivateKeyFromPem(FileName,pwd)
	if err != nil{
		return nil, err
	}
	key:=PrivateKey{}
...
func writePublicKeytoMem(key *tjfoc_gm_sm2.PublicKey,pwd []byte) ([]byte, error) {
